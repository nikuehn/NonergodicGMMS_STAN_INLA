...
axis(1); axis(2)
save(mesh1, file = 'RESULTS_INLA/mesh.Rdata')
...
fit_inla_spatial_ask <- inla(form, 
                         data = inla.stack.data(stk_eq_stat_cell), 
                         control.predictor = list(A = inla.stack.A(stk_eq_stat_cell)),
                         control.compute = list(dic = TRUE, cpo = TRUE, waic = TRUE, config = TRUE),
                         family="gaussian",
                         control.fixed = prior.fixed,
                         control.family = list(hyper = list(prec = prior_prec)),
...
#save(fit_inla_ask,
#     fit_inla_spatial_ask,
#     file = file.path('RESULTS_INLA',sprintf('fit_inla_prior_ask_%s.Rdata',period)))


### assess results

# get a summary of the models
summary(fit_inla_ask)
summary(fit_inla_spatial_ask)

# get the posterior dstribution of phi_SS, which can be calculated
# by transforming the marginal distribution of the log precision of the 
# observation likelihood (the log precision is the parameter that is estimated)
post_phi0_ask <- inla.tmarginal(function(x) sqrt(exp(-x)), 
                                fit_inla_spatial_ask$internal.marginals.hyperpar$`Log precision for the Gaussian observations`)
plot(post_phi0_ask)

# get posterior distribution of tau_0 and phiS2S0
post_tau0_ask <- inla.tmarginal(function(x) sqrt(exp(-x)), 
                                fit_inla_spatial_ask$internal.marginals.hyperpar$`Log precision for eq`)
plot(post_tau0_ask)

post_phis2s0_ask <- inla.tmarginal(function(x) sqrt(exp(-x)), 
                                fit_inla_spatial_ask$internal.marginals.hyperpar$`Log precision for stat`)
plot(post_phis2s0_ask)

# get range and standard deviation of systematic event term hyperparameters
post_rangeeq_ask <- inla.tmarginal(function(x) exp(x), 
                                   fit_inla_spatial_ask$internal.marginals.hyperpar$`log(Range) for idx.eq`)
post_psieq_ask <- inla.tmarginal(function(x) exp(x), 
                                 fit_inla_spatial_ask$internal.marginals.hyperpar$`log(Stdev) for idx.eq`)

# get range and standard deviation of systematic station term hyperparameters
post_rangestat_ask <- inla.tmarginal(function(x) exp(x), 
                                   fit_inla_spatial_ask$internal.marginals.hyperpar$`log(Range) for idx.stat`)
post_psistat_ask <- inla.tmarginal(function(x) exp(x), 
                                 fit_inla_spatial_ask$internal.marginals.hyperpar$`log(Stdev) for idx.stat`)

# get meab (fixed effect) and standard deviation of anelastic attenuation effects
post_psicell_ask <- inla.tmarginal(function(x) sqrt(exp(-x)), 
                                   fit_inla_spatial_ask$internal.marginals.hyperpar$`Log precision for idx_cell`)
post_linr_ask <- inla.tmarginal(function(x) -x, 
                                fit_inla_spatial_ask$marginals.fixed$R)


# plot mean and sd of spatially varying event terms
local.plot.field(fit_inla_spatial_ask$summary.random$idx.eq$mean, mesh1, xlim = c(-126,-114), ylim = c(30,42))
local.plot.field(fit_inla_spatial_ask$summary.random$idx.eq$sd, mesh1, xlim = c(-126,-114), ylim = c(30,42))

# get the cell-specific attenuation parameters
tmp <- fit_inla_spatial_ask$summary.random$idx_cell
cell_attn <- tmp[(n_rec + 1):(n_rec + n_cell),]
cell_attn[,1] <- cell_idx
