...
vs30 scaling is correlated with Matern 1/2

to avoid problems of identfiability, the length scale of the constant is constrained to be
smaller than the legth scale for the Vs30 scaling
...
parameters {
  ...
  positive_ordered[2] rho_stat; // length scale for spatially correlated station terms and Vs30 scaling
  vector<lower=0>[2] theta_stat; // std of spatially correlated station terms and Vs30 scaling
  ...
  // latent variable station contributions to GP
  {
    ...
    for(i in 1:NSTAT) {
      for(j in i:NSTAT) {
        ...
        real c_stat = square(theta_stat[1]) * exp(-d_s/rho_stat[1]);
        real c_vs = lnVS[i] * lnVS[j] * square(theta_stat[2]) * exp(-d_s/rho_stat[2]);
  
        cov_stat[i,j] = c_stat + c_vs;
